...
const DOTS_REGEX = /\./g;
const outFile = path.join(__dirname, '../src/typings/obsWebsocket.ts');
...
function parseApi(raw: RawComments): void {
  ...
  Object.values(raw.typedefs).forEach(typedef => {
    let typedefString = `export type ${typedef.typedefs[0].name} = {`;
...
/* tslint:disable:no-trailing-whitespace no-dead-reference */
  const sourceCode = `/// THIS FILE IS AUTOMATICALLY GENERATED
/// DO NOT EDIT
